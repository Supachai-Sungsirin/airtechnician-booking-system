...
const Step3ProfessionalInfo = ({ formData, setFormData }) => {
  ...
  const serviceTypes = [
    ...
    { value: "refill", label: "เติมน้ำยา" },
  ]

  const handleServiceAreaChange = (e) => {
    const { value, checked } = e.target
    setFormData((prev) => {
      const updatedAreas = checked ? [...prev.serviceArea, value] : prev.serviceArea.filter((area) => area !== value)
  ...
  const handleServiceTypeChange = (e) => {
    const { value, checked } = e.target
    setFormData((prev) => {
      const updatedServices = checked ? [...prev.services, value] : prev.services.filter((service) => service !== value)
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-gray-600">กรอกข้อมูลเกี่ยวกับการทำงานของคุณ</p>
      </div>
      ...
      <div>
        ...
        <p className="text-xs text-gray-500 mt-2">เลือกแล้ว {formData.services.length} บริการ</p>
      </div>

      <div>
        <label className="block text-sm font-medium text-gray-700 mb-3">
          พื้นที่ให้บริการ (เขตในกรุงเทพฯ) <span className="text-red-500">*</span>
        </label>
        ...
        <div className="max-h-64 overflow-y-auto space-y-2 border border-gray-200 rounded-lg p-3">
          {serviceAreas.filter((area) => area.toLowerCase().includes(searchQuery.toLowerCase())).length > 0 ? (
            serviceAreas
        ...
        <p className="text-xs text-gray-500 mt-2">เลือกแล้ว {formData.serviceArea.length} เขต</p>
      </div>
