...
import api from "../../services/api"; // ตรวจสอบ path ของ api.js ให้ถูกต้อง
...
export default function BookingModal({ onClose, onSuccess }) {
  ...
  const [userProfile, setUserProfile] = useState(null); // [ใหม่] เก็บข้อมูล user profile ทั้งหมด
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        console.log("Fetched Services (all active from API):", serviceResponse.data.filter((s) => s.active));

        // ดึงข้อมูลผู้ใช้ที่ล็อกอินอยู่
        const userResponse = await api.get("/auth/me");
        const userData = userResponse.data;
        setUserProfile(userData); // เก็บข้อมูลโปรไฟล์ผู้ใช้

        // ตรวจสอบว่ามีข้อมูลที่อยู่และเขต/อำเภอครบถ้วนหรือไม่
        ...
        // ถ้าดึงไม่ได้ ให้เคลียร์ข้อมูล address/district เพื่อให้ช่องกรอกแสดงขึ้นมา
        setFormData(prevFormData => ({
            ...prevFormData,
            address: "",
            district: "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const findTechnicianResponse = await api.post("/api/technicians/find", {
          serviceIds: serviceIdsToFindTechnicians,
          district: district, 
          requestedDateTime: formData.requestedDateTime,
      });

      const foundTechnicians = findTechnicianResponse.data.technicians;
      let technicianMessage = '';
      if (foundTechnicians && foundTechnicians.length > 0) {
          technicianMessage = `\n\nช่างที่เหมาะสมที่พบในเขตของคุณ:\n`;
          foundTechnicians.forEach(tech => {
              technicianMessage += `- ${tech.name} (${tech.phone})${assignedTechnician && assignedTechnician._id === tech._id ? ' (ช่างที่ได้รับมอบหมายทันที)' : ''}\n`;
          });
      } else {
          technicianMessage = '\n\nไม่พบช่างที่เหมาะสมในเขตของคุณในขณะนี้ ระบบจะดำเนินการค้นหาต่อไป';
      }

      alert(
        `${message}\n\nราคารวม: ฿${totalPrice.toLocaleString()}` + 
        `${assignedTechnician ? `\n\nช่างที่ได้รับมอบหมายทันที:\n${assignedTechnician.name}\nเบอร์โทร: ${assignedTechnician.phone}` : ''}` +
        `${technicianMessage}` 
      );
      ...
      console.error("Error creating booking or finding technicians:", error);
  ...
  // [ใหม่] ตรวจสอบว่าข้อมูลที่อยู่และเขตถูกดึงมาจากโปรไฟล์และครบถ้วนหรือไม่
  const isAddressFromProfile = userProfile && userProfile.address && userProfile.district && userProfile.province && userProfile.postalCode;
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
            <div className="flex gap-3">
              ...
              <div className="text-sm text-blue-800">
                ...
                <p className="mt-1">
                  เมื่อคุณจองบริการ ระบบจะหาช่างที่ให้บริการในเขตของคุณโดยอัตโนมัติ
                </p>
              </div>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              {formData.services.map((service, index) => {
                ...
                const hasBtuOptions =
                  selectedService && selectedService.options.some((opt) => opt.btuRange);
                          ...
                          {hasBtuOptions ? (
                            <div>
                              <select
                                ...
                                onChange={(e) =>
                                  updateServiceItem(index, "btuRange", e.target.value)
                                }
                                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
                              >
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, requestedDateTime: e.target.value })
                }
            ...
            {/* [แก้ไข] เงื่อนไขการแสดงช่องกรอกที่อยู่ */}
            {isAddressFromProfile ? (
              // แสดงข้อมูลที่อยู่จากโปรไฟล์
              <>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    ที่อยู่
              ...
              // แสดงช่องกรอกที่อยู่เพื่อให้ผู้ใช้กรอกเอง
              <>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    ที่อยู่ *
