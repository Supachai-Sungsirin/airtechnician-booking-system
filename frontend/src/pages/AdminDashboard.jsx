...
export default function AdminDashboard() {
  ...
  const [adminName, setAdminName] = useState("กำลังโหลด...");
  ...
  useEffect(() => {
    const fetchAdminData = async () => {
      try {
        const res = await api.get("/auth/me"); // Endpoint สำหรับดึงข้อมูลผู้ใช้ปัจจุบัน
        setAdminName(res.data.fullName || "Admin");
      } catch (err) {
        console.error("Error fetching admin profile:", err);
        // หากดึงข้อมูลไม่ได้ ให้ออกจากระบบ
        handleLogout();
      }
    };
    fetchAdminData();
          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">ระบบจัดการหลังบ้าน</p>
              </div>
            </div>
